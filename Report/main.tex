...
\newcommand{\thesisAuthor}{Ulf Nore, Anders Røsæg Pedersen}
\newcommand{\thesisTitle}{Blind Soure Separation}
...
\vfill
\begin{center}
\includegraphics[width=4cm]{Figures/Ntnu-logo.png}
\end{center}
\end{titlepage}
...
problem. Chapter \ref{ss_bss} is devoted to the SSBSS problem.
...
of latent variables representing the sources. The first approach\cite{roweisMaxVQ} 
represents each of sources by a Gaussian mixture model (GMM) where each component
...
signals. Roweis \cite{roweisOneMic} therefore proposes an extended hidden markov approach
...
After screening the results against the different criteria set in appendix \ref{protocol}, the papers that were brought on for further consideration are shown in table \ref{tab:2.2} at the end of this chapter.
...
\begin{table}[ht]
    \begin{tabular}{|p{6cm}|p{4cm}|p{2cm}|}
        ...
        Hidden Markov model decomposition of speech and noise & Varga, A. P., and R. K. Moore & 1990\\ \hline
        Factorial models and refiltering for speech separation and denoising & Roweis, Sam T. & 2003\\ \hline
    \end{tabular}
\caption{Papers that satisfied the criteria}
\label{tab:selectedPapers}
\end{table}
...
Roweis\cite{roweisOneMic} proposes a technique called refiltering, where the idea is to separate sources in a mixed or corrupted recording. This is achieved through non stationary masking of the different frequency sub-bands from the target recording. Different sources may be isolated in the recording by changing the masking parameters. Using regularities in the spectrogram produced by a recording, it is possible to set the masking parameters, eg. common onset and offset. 
...
mathematics (see for instance \cite{ESL}). Rather, we will proceed to illustrate an
application of PCA to BSS.
...
mixing matrix using PCA. Figure \ref{pca_data_eigs} shows the original
data plottet against the two first eigenvectors, and looking at figure
\ref{pca_eig_projection} we see that after PCA, the datapoints have been
rotated by projection onto these eigenvectors.
...
differential entropy \cite{ESL}, maximum kurtosis \cite{ESL} and 
...
in greater detail in \cite{comon94} \cite{hyvarinen2001}.
...
From Equation \ref{bayes}, we can set up the maximum likelihood function:
...
the pseudocode Figure \ref{mlica_code}. 
...
For a more sophisticated ICA algorithm, we refer to Hyvarinen's FastICA\cite{fastICA}.
...
\chapter{Single Sensor Blind Source Separation}\label{ss_bss}
...
problem proposed by Roweis \cite{roweisMaxVQ} that relies on a
...
extended hidden markov model proposed by Roweis\cite{roweisOneMic}, before we illustrate the
...
for instance Figure \ref{fig:timeFreqRep}, sound signals are often non-stationary,
...
concentrated in the upper part of the spectrogram and vice versa.
...
har the joint distribution (Figure \ref{gaussmix_eqn}) then we say $X$ and $Z$
...
separated. The canonical training method
used in these models is the expectation maximization (EM) algorithm. 
...
convergence as described in Russel and Norvig\cite{norvig_russel}.
...
is based on Roweis\cite{roweisMaxVQ}, but in this step we will ignore the branch-and-bound procedure,
...
Roweis\cite{roweisMaxVQ} describes a branch-and-bound methodbased on
...
A hidden markov model (HMM), described in \cite{norvig_russel}, is a probabilistic model relating two sequences of
...
the past given the present''. Another common simplification is known as the 
\emph{markov sensor model assumption}:
...
\subsection{A HMM Blind Source Separation Framework}

We will now describe the factoral HMM for blind source separation as
put forward by Roweis\cite{roweisOneMic} in a two signal setting. We adopt the following
...
emission probabilities $P(X|Z)$. While Roweis
...
We follow Roweis\cite{roweisMaxVQ} in estimating a Gaussian mixture model with a single
...
signals, Roweis\cite{roweisMaxVQ} argues for a model whereby the observed value equals the
...
We recognize this as the logmax approximation discussed in Section \ref{logmaxapprox}.
The key augmentation to the the latent variable MAXVQ model is therefore the addition
of a transition model. 


\section{Results}\label{ssbss_results}

To illustrate the MAXVQ method discussed in this chapter, we return to our base case signals consisting
...
In our experiment in this section we have split the data in half, one training set and one test set. 
The set is preprocessed by constructing
...
a model with $256$ components for the voice signal and $128$ for the music\footnote{In Roweis\cite{roweisMaxVQ}, an experiment is 
...
Figures \ref{voice_result}-\ref{music_result} shows the recovered voice and music signals for the mixture in 
the lowermost plot in Figure \ref{spect_testset}. While it is hard to appreciate the performance of the 
of the algorithm judging from just a set of spectrograms, we can distinguish some important features that 
are apparent. 

\begin{itemize}
\item The second and third utterance of the speaker which was close to invisible in the mixture spectrogram is now visible in Figure \ref{voice_result}.
